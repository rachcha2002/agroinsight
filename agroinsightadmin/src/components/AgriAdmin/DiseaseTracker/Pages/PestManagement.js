...
const PestManagement = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/disease/complaints`);
  ...
  const handleEdit = async () => {
    try {
      const response = await axios.put(`${process.env.REACT_APP_BACKEND_URL}/api/disease/complaints/${selectedComplaint._id}`, formData);
      if (response.status === 200) {
        // Update the complaint data in the table
        setComplaintData((prevData) =>
          prevData.map((complaint) =>
            complaint._id === selectedComplaint._id ? { ...complaint, ...formData } : complaint
          )
        );
        setShow(false);
  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/disease/complaints/${selectedComplaint._id}`);
      if (response.status === 200) {
        // Remove the complaint from the table
        setComplaintData(complaintData.filter((complaint) => complaint._id !== selectedComplaint._id));
        setShow(false);
          ...
          <Modal.Body>
            <div className="mb-3 text-center">
              {selectedComplaint.imageURL && (
                <Image src={selectedComplaint.imageURL} fluid rounded />
              )}
            </div>
            ...
            <Row>
              ...
              <Col md={6}>
                <Form.Group controlId="dateOfComplaint" className="mt-3">
                  ...
                  <Form.Control
                    type="text"
                    value={moment(selectedComplaint.dateOfComplaint).format('YYYY-MM-DD')}
                    readOnly
                  />
                </Form.Group>
              </Col>
